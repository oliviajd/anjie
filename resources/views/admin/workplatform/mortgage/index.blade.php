    ...
    <div class="modal-dialog" role="document">  
        <div class="modal-content">  
            <div class="modal-header">  
                ...
                <h4 class="modal-title" id="myModalLabel">抵押登记</h4>  
            </div>  
                ...
                <table id="example2" class="table table-bordered table-hover" style="font-size: 12px;">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>车辆登记证书</td>
                            <td>★委托书编号</td>
                            <td><input type="text" name="mandate_number" class="form-control mandate_number" placeholder=""></td>
                        </tr>
                        <tr>
                            <td>抵押人员</td>
                            <td><input type="text" name="transactor" class="form-control transactor" placeholder=""></td>
                            <td>★抵押日期</td>
                            <td><input id="transact_time" name="transact_time" type="text" class="form-control pull-right transact_time"></td>
                        </tr>
                        <tr>
                            <td>备注</td>
                            <td colspan="3"><textarea name="mortgage_description" class="comments" cols="75" rows="4" placeholder=""></textarea></td>
                        </tr>
                    </tbody>   
                </table> 
            </div>  
...
<script type="text/javascript">
    ...
    var task_deal = function (id, item_instance_id) {
        ...
        $('[name=mandate_number]').val(row['mandate_number'])
        $('[name=transactor]').val(row['transactor'])
        $('[name=transact_time]').val(row['transact_time'])
        $('[name=mortgage_description]').val(row['mortgage_description'])
        ...
        $('#assign_confirm').click(function () {
            if (checkempty('mandate_number,transact_time') == false) {
                var modal = $('#myModal');
            ...
            var data = {
                ...
                'token': token,
                // 'is_mortgage': $("[name=is_mortgage]:checked").val(),
                'mandate_number': $("[name=mandate_number]").val(),
                'transactor': $("[name=transactor]").val(),
                'transact_time': $("[name=transact_time]").val(),
                'mortgage_description': $("[name=mortgage_description]").val(),
            };
            $.ajax({
                url: '/Api/workplatform/mortgage',
                type: 'post',
                data: data,
                dataType: 'json',
                'success': function (r) {
    ...
    $(function () {
        ...
        $('#transact_time').daterangepicker({singleDatePicker: true, timePickerIncrement: 30, format: 'MM/DD/YYYY h:mm A'});
