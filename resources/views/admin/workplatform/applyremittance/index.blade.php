    ...
    <div class="modal-dialog" role="document">  
        <div class="modal-content">  
            <div class="modal-header">  
                ...
                <h4 class="modal-title" id="myModalLabel">申请打款</h4>  
            </div>  
            <div class="modal-body"> 
                <table id="example2" class="table table-bordered table-hover" style="font-size: 12px;">
                    <tbody>
                        ...
                        <tr>
                            <td>★贷款本金</td>
                            <td><input type="text" name="loan_principal" class="form-control loan_principal" placeholder=""></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>★打款户名</td>
                            <td><input type="text" name="finance_name" class="form-control finance_name" placeholder=""></td>
                            <td>★打给车行</td>
                            <td><input type="text" name="finance_driving" class="form-control pull-right finance_driving"></td>
                        </tr>
                        <tr>
                            <td>★账号</td>
                            <td><input type="text" name="finance_account" class="form-control finance_account" placeholder=""></td>
                            <td>★开户行</td>
                            <td><input type="text" name="finance_deposit_bank" class="form-control finance_deposit_bank" placeholder=""></td>
                        </tr>
                        <tr>
                            <td>★打款金额</td>
                            <td><input type="text" name="finance_amount" class="form-control finance_amount" placeholder=""></td>
                            <td>★打款日期</td>
                            <td><input type="text" name="finance_date" class="form-control finance_date" placeholder=""  id="finance_date"></td>
                        </tr>
                        <tr>
                            <td>★备注</td>
                            <td colspan="3"><textarea name="finance_apply_description" class="comments finance_apply_description" cols="75" rows="4" placeholder=""></textarea></td>
                        </tr>
                    </tbody>   
                </table> 
            </div>  
...
<script type="text/javascript">
    ...
    var task_deal = function (id, item_instance_id) {
        ...
        $('#assign_confirm').click(function () {
            if (checkempty('loan_principal,finance_name,finance_driving,finance_account,finance_deposit_bank,finance_amount,finance_date,finance_apply_description') == false) {
                var modal = $('#myModal');
            ...
            var data = {
                ...
                'token':token,
                'loan_principal': $("[name=loan_principal]").val(),
                'finance_name': $("[name=finance_name]").val(),
                'finance_driving': $("[name=finance_driving]").val(),
                'finance_account': $("[name=finance_account]").val(),
                'finance_deposit_bank': $("[name=finance_deposit_bank]").val(),
                'finance_amount': $("[name=finance_amount]").val(),
                'finance_date': $("[name=finance_date]").val(),
                'finance_apply_description': $("[name=finance_apply_description]").val(),
            };
            $.ajax({
                url: '/Api/workplatform/applyremittance',
                type: 'post',
                data: data,
                dataType: 'json',
                'success': function (r) {
    ...
    $(function () {
        ...
        $('#finance_date').daterangepicker({singleDatePicker: true, timePickerIncrement: 30, format: 'MM/DD/YYYY h:mm A'});
        ...
        var table = $("#example1").DataTable({
            ...
            "columns": [
                ...
                },
                {
                    "orderable": false,
                    "data": function (row) {
                        return '<button type="button" class="btn btn-default" onclick="task_deal(' + row.id + ',' + row.item_instance_id + ');">处理</button>';
                    }
                },
            ],
//                    "order": [[1, 'asc']]
